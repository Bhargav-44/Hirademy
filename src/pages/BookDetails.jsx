import React, { useEffect } from "react";
import { useGlobalContext } from "../Context";
import { useNavigate } from "react-router-dom";

const BookDetails = () => {
  const { bookDetail, setBookDetail } = useGlobalContext();
  const navigate = useNavigate();

  useEffect(() => {
    localStorage.setItem("bookDetail", JSON.stringify(bookDetail));
  }, [bookDetail]);

  console.log(bookDetail);

  if (!bookDetail || !bookDetail.authors) {
    return (
      <div
        className="flex flex-col items-center justify-center h-screen"
        style={{
          backgroundImage:
            "linear-gradient(to left top, #6f5f68, #85566a, #9c4a64, #b13a56, #c22541)",
        }}
      >
        <p className="text-2xl font-montserrat font-bold mb-4 text-white">Hold a sec!!</p>
        <svg
          aria-hidden="true"
          className="w-16 h-16 text-gray-200 animate-spin dark:text-gray-600 fill-white"
          viewBox="0 0 100 101"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
            fill="currentColor"
          />
          <path
            d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
            fill="currentFill"
          />
        </svg>
      </div>
    );
  }

  const handleBack = () => {
    navigate("/");
    setBookDetail({});
  };

  return (
    <div
      className="flex flex-col items-center justify-center h-screen"
      style={{
        backgroundImage:
          "linear-gradient(to left top, #6f5f68, #85566a, #9c4a64, #b13a56, #c22541)",
      }}
    >
      <p className="text-4xl font-bold mb-10 text-center text-white w-2/3">
        Learn more about, {bookDetail.title}
      </p>
      <div className="w-auto font-montserrat bg-[#CDCBDA] shadow-xl shadow-gray-400 p-6 rounded-lg ">
        <p className="text-lg  mb-2">
          <span className="mr-1 font-semibold">ISBN:</span>
          {bookDetail.isbn}
        </p>
        <p className="text-lg  mb-2">
          <span className="mr-1 font-semibold">Page Count:</span>
          {bookDetail.pageCount === 0
            ? `Finding the count seems difficult!!`
            : bookDetail.pageCount}
        </p>
        <div className="flex items-center mb-4">
          <span className="text-lg font-semibold mr-1">
            {bookDetail.authors.length === 1 ? `Author: ` : `Authors`}
          </span>
          {bookDetail.authors.map(
            (author, i) =>
              author && (
                <React.Fragment key={i}>
                  <p className="text-base text-gray-700">{author}</p>
                  {i !== bookDetail.authors.length - 1 && (
                    <span className="text-base text-gray-500 mr-1">,</span>
                  )}
                </React.Fragment>
              )
          )}
        </div>

        <button
          className="py-3 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-black text-white shadow-sm hover:bg-gray-500 hover:text-black   "
          onClick={handleBack}
        >
          <svg
            className="flex-shrink-0 size-4"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            style={{ transform: "rotate(180deg)" }}
          >
            <path d="M5 12h14"></path>
            <path d="m12 5 7 7-7 7"></path>
          </svg>
          Go Back
        </button>
      </div>
    </div>
  );
};

export default BookDetails;
